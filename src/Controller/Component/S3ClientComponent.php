...
class S3ClientComponent extends Component
{
    ...
    public function getList($buket_name=null, $dir=null, $get_max=100)
    {
      try {
        ...
        foreach ($list_obj['Contents'] as $file) {
          if(mb_substr($file['Key'], -1) !== "/"
            && (!$dir || ($dir && strpos($file['Key'], sprintf('%s/', $dir)) !== false))) {
            $result[] = $file['Key'];
          }
        }
        return $result;
    ...
    public function copyFile($s3_file_path, $s3_copy_file_path, $bucket_name_from=null, $bucket_name_to=null)
    {
      try {
        ...
        , 'CopySouece'  => sprintf('%s/%s', $bucket_name_from, $s3_file_path)
        ));
        return $result;
      } catch (S3Exception $e) {
        echo $e->getMessage();
    ...
    public function createMultipleObjects($file_list)
    {
    ...
    public function chkDirHandle($file_list, $local_dir_path)
    {
    ...
    public function chkDir($local_dir_path) 
    {
