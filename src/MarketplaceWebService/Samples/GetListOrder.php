...
$fromGetDay = 3;
$currentDir = getcwd();
define("DIR_DATA_LIST",     $currentDir);
$country = '_JP';
//$country = '_AU';
//$country = '_US';
$outFile = sprintf('%s/%s/ORDER_%s_%s.csv', DIR_DATA_LIST, $country, date('Ymd'), date('His'));
...
$params['LastUpdatedAfter']  = gmdate('Y-m-d\TH:i:s\Z', strtotime("- $fromGetDay days"));
$params['MarketplaceId.Id.1']= $marketplace_id;

$baseurl = $baseurl . 'Orders/' . $params['Version'];
...
$getOrderId = '';
foreach($amazon_xml->ListOrdersResult->Orders->Order as $responseOrder){
    $getOrderId = $getOrderId . $responseOrder->AmazonOrderId . ':';
}

printf ("SUCCESS : MWS ListOrders completed! OrderId=%s\n", $getOrderId );
printf ("Filename: %s\n", $outFile);
